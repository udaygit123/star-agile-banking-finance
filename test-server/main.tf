resource "aws_instance" "test-server1" {
  ...
  provisioner "local-exec" {
   ...
   provisioner "local-exec" {
  command = "ansiblePlaybook disableHostKeyChecking: true, installation: 'ansible', inventory: '/etc/ansible/hosts', playbook: '/var/lib/jenkins/workspace/Banking-F-Project/test-server/myansible-playbook.yml'"
  } 
}
